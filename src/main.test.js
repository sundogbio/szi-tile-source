...
test('Test url mapper', () => {
  const mapper = new UrlMapper('http://localhost:5173/examples/compressed/mixmas.szi');
  expect(mapper.sziFilenameWithoutSuffix).toEqual('mixmas');
  expect(mapper.sziPathWithoutFilename).toEqual('/examples/compressed/');
  expect(mapper.pathInSziFromDziUrl('http://localhost:5173/examples/compressed/mixmas/mixmas_files/0_0.jpg')).toEqual(
    'mixmas/mixmas_files/0_0.jpg',
  );
  expect(mapper.dziXmlUrl()).toEqual('http://localhost:5173/examples/compressed/mixmas/mixmas.dzi');
});
...
test('Test construction', { timeout: 300_000 }, async () => {
  ...
  const sziTileSource = await OpenSeadragon.SziTileSource.createSziTileSource(
    'http://localhost:5173/examples/compressed/mixmas.szi',
  );

  expect(sziTileSource.getTileUrl(0, 0, 0)).toEqual(
    'http://localhost:5173/examples/compressed/mixmas/mixmas_files/0/0_0.jpeg',
  );
  expect(sziTileSource.getTileUrl(10, 55, 33)).toEqual(
    'http://localhost:5173/examples/compressed/mixmas/mixmas_files/10/55_33.jpeg',
  );
});
